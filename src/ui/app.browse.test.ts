...
describe("Nutka TUI: browse", () => {
  ...
  test("opens artist sections, paginates the selected section, and restores history", async () => {
    ...
    const centerRow = (line: string): string => line.match(/│ │ │(.*?)│ │ │/u)?.[1] ?? line
    const songRow = artistPage.split("\n")
      .map(centerRow)
      .find((line) => line.includes("First Track") && line.includes("song"))
    const albumRow = artistPage.split("\n")
      .map(centerRow)
      .find((line) => line.includes("The Album") && line.includes("album"))
