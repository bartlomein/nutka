...
Apple private key in the open-source Nuta client. Authorization capabilities,
...
exactly to `127.0.0.1`. Nuta creates a five-minute session and opens the
...
the capability-holding Nuta process, validated against Apple's storefront
endpoint, and stored in the operating-system credential manager. Nuta then
...
`$XDG_STATE_HOME/nuta/auth.log` (falling back to
`~/.local/state/nuta/auth.log`). Entries contain no URLs, request bodies,
headers, capabilities, or Apple tokens.

The loopback-only `/playback` page is a static MusicKit host for Nuta's headless
